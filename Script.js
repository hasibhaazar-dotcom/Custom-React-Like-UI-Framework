 ...
 * Core features: Virtual DOM, Diff/Reconciliation, Component System, Hooks (useState, useEffect)
 */
...
export function diff(parent, dom, oldVNode, newVNode) {
    if (!dom) {
...
import { createElement, render, useState, useEffect } from './framework.js';

function TodoApp() {
    const [todos, setTodos] = useState(() => {
        const saved = localStorage.getItem('custom_framework_todos');
        return saved ? JSON.parse(saved) : [
            { id: 1, text: 'Build Virtual DOM engine', completed: true },
            { id: 2, text: 'Verify Diff & Reconciliation loops', completed: false }
        ];
    });
